...
_LOGICAL_PATHS = {
    "feature_spec": ".rpgkit/data/feature_spec.json",
    "feature_build": ".rpgkit/data/feature_build.json",
    "feature_refactor": ".rpgkit/data/feature_tree.json",
}
...
def _install_sigint_handler() -> None:
    def _handle(signum: int, frame: Any) -> None:
        print("\n[feature_construct] interrupted — rerun `rpgkit script feature_construct.py` to resume.")
        sys.exit(130)
...
def _print_failure_hint(stage: Stage, rc: int, *, phase: str) -> None:
    ...
    debug_cmd = " ".join(["rpgkit", "script", stage.build_script, *debug_args])
    print(file=sys.stderr)
    print(f"X {stage.name} {phase} failed (exit {rc})", file=sys.stderr)
    print("  Resume :  rpgkit script feature_construct.py", file=sys.stderr)
    print(f"  Debug  :  {debug_cmd}", file=sys.stderr)
    print("  Status :  rpgkit script feature_construct.py --check-only", file=sys.stderr)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not runnable:
        ...
        print("Use `rpgkit script feature_construct.py --force` to rebuild from scratch.")
        print("Next: `/rpgkit.plan` to build the RPG planning pipeline.")
        return 0
    ...
    print("Next: `/rpgkit.plan` to build the RPG planning pipeline.")
    print("Optional: expand features, or run `/rpgkit.feature_edit <instructions>` for small tree adjustments.")
    print("Granular debug commands remain available: `/rpgkit.feature_spec`, `/rpgkit.feature_build`, `/rpgkit.feature_refactor`.")
    return 0
