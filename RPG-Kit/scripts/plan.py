...
five sequential slash-commands ``/rpgkit.build_skeleton`` →
``/rpgkit.build_data_flow`` → ``/rpgkit.design_base_classes`` →
``/rpgkit.design_interfaces`` → ``/rpgkit.plan_tasks``.
...
Sub-scripts are invoked via ``rpgkit script <name>`` when the
``rpgkit`` CLI is on ``$PATH`` (so each stage gets its own
``logs/<stage>.log`` and inner-git snapshot, courtesy of the
dispatcher).  When ``rpgkit`` is missing, the script falls back
...
# rpgkit_cli/core_pack/scripts/ in the installed wheel).
...
def _resolve_invoker() -> list[str]:
    ...
    Prefer the ``rpgkit`` CLI so the dispatcher tees each stage's
    output to ``~/.rpgkit/workspaces/<hash>/logs/<stem>.log`` and
    snapshots the inner git repo automatically.  Fall back to a
    direct python invocation when ``rpgkit`` is not on ``$PATH``.
    """
    rpgkit = shutil.which("rpgkit")
    if rpgkit:
        return [rpgkit, "script"]
...
def _install_sigint_handler() -> None:
    def _handle(signum: int, frame: Any) -> None:  # noqa: ARG001
        print("\n[plan] interrupted — rerun `rpgkit script plan.py` to resume.")
        sys.exit(130)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if head.type == "error" and not args.dry_run:
        ...
        print(
            "Run `/rpgkit.feature_refactor` first to produce "
            "`feature_tree.json`, then re-run `/rpgkit.plan`.",
            file=sys.stderr,
        )
        return 2
    ...
    if not runnable:
        ...
        print("Use `rpgkit script plan.py --force` to rebuild from scratch.")
        return 0
...
def _print_failure_hint(stage: Stage, rc: int, *, phase: str) -> None:
    ...
    print("  Resume :  rpgkit script plan.py", file=sys.stderr)
    print(
        f"  Debug  :  rpgkit script {debug_script} --verbose",
        file=sys.stderr,
    )
    print(
        "  Status :  rpgkit script plan.py --check-only",
        file=sys.stderr,
    )
