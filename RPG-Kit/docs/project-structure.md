...
- A single `.git` repository tracks user-owned code and any RPG-Kit files the user chooses to commit. **Runtime data, logs, reports, and the inner-git snapshot repo all live outside the workspace** under `~/.rpgkit/workspaces/<hash>/`, so nothing generated by RPG-Kit pollutes your repo or accidentally gets committed.
...
To discover the hash for the current workspace, run any rpgkit command with `--verbose`, or use Python:

```bash
python3 -c 'import hashlib, os, sys; print(hashlib.sha256(os.path.realpath(sys.argv[1]).encode()).hexdigest()[:12])' .
```
