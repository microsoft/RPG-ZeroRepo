    ...
    def _execute_in_docker(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            if tool_call.name == "bash":
                ...
                # Keep as string to use persistent shell (handles interactive commands better)
                command_to_run = command_value
                print(f"[DEBUG] Docker bash command: {command_value}")
            ...
            # --- Rule2 : Handling str_replace_based_edit_tool ---
                ...
                # 构造 argv list（每个 option 和 value 为独立元素）
                ...
                # cmd_parts 现在是一个 list，可以直接传给 docker exec_run / exec_create
                command_to_run = cmd_parts
                # print(f"Built command for Docker execution: {command_to_run}")

                # 示例：通过 docker-py 直接执行（如果你的 DockerManager 支持 exec_in_container）
                # exit_code, output = self._docker_manager.exec_in_container(command_to_run)
                # 如果你的 execute 接口仍然期望字符串，请改造 execute 以接受 list（推荐）
            ...
            # --- Rule 3: Handling json_edit_tool ---
                ...
                for key, value in processed_args.items():
                    ...
                    if key == "value":
                        ...
                        flag = f"--{key}"
                        cmd_parts.append(flag)
                        cmd_parts.append(json_string_value)
