...
"""
    ...
    def get_parameters(self) -> list[ToolParameter]:
        ...
        return [
            ToolParameter(
                name="think",
                type="string",
                description="Briefly explain, based on the current situation, why you are invoking this tool and choosing this command.",
                required=True,
            ),
            ToolParameter(
                name="command",
    ...
    def validate_path(self, command: str, path: Path):
        ...
        if command == "create":
            # 1) 目标路径本身如果已经存在，就不允许覆盖
            if path.exists():
                raise ToolError(
                    f"File already exists at: {path}. Cannot overwrite files using command `create`."
                )
            # 2) 向上找最近一个已存在的祖先路径，确认它是目录而不是文件
            ancestor = path.parent
            while ancestor != ancestor.parent and not ancestor.exists():
                ancestor = ancestor.parent
            if ancestor.exists() and not ancestor.is_dir():
                # 例如：/repo/file.txt/foo/bar.py，这里 file.txt 是文件，不能在下面建目录
                raise ToolError(
                    f"Cannot create file at: {path}. "
                    f"Ancestor path {ancestor} is a file, not a directory."
                )
            # 其他情况（包括父目录不存在）交给 _create_handler 里的 mkdir 处理
            return
    ...
    async def _view(self, path: Path, view_range: list[int] | None = None) -> ToolExecResult:
        ...
        if view_range:
            ...
            TOL = 100  # 允许一定范围内“超出文件行数”的请求
            # 只有在 final_line 不是 -1 时才需要做这些检查
            if final_line != -1:
    ...
    def _create_handler(self, arguments: ToolCallArguments, _path: Path) -> ToolExecResult:
        ...
        try:
            ...
            return ToolExecResult(
                error=f"Failed to create parent directories for {_path}: {e}",
                error_code=-1,
            )
