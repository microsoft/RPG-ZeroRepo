...
class BaseAgent(ABC):
    ...
    async def _run_llm_step(
        ...
        if self.llm_indicates_task_completed(llm_response):
            if self._is_task_completed(llm_response):
                ...
                execution.agent_state = AgentState.RUNNING
                # For OpenAI-compatible providers, if the LLM response contains tool_calls,
                # we must first respond with tool role messages before sending user messages.
                return self._build_task_incomplete_messages(llm_response)
        else:
            tool_calls = llm_response.tool_calls
            return await self._tool_call_handler(tool_calls, step, llm_response)
    ...
    def _is_openai_compatible_provider(self) -> bool:
        """Check if the current provider requires OpenAI-style tool response messages."""
        provider = self._model_config.model_provider.provider.lower()
        # OpenAI and OpenAI-compatible providers require 'tool' role after tool_calls
        return provider in ("openai", "azure", "openrouter", "ollama")

    def _build_task_incomplete_messages(self, llm_response: LLMResponse) -> list[LLMMessage]:
        """Build messages to indicate task is incomplete, handling tool_calls if present.

        For OpenAI-compatible providers, if the LLM response contains tool_calls,
        we must first respond with tool result messages before sending user messages.
        """
        messages: list[LLMMessage] = []

        # If there are tool_calls in the response and we're using an OpenAI-compatible provider,
        # we need to provide tool result messages first
        if llm_response.tool_calls and self._is_openai_compatible_provider():
            for tool_call in llm_response.tool_calls:
                # Create a placeholder tool result indicating the task was marked complete prematurely
                tool_result = ToolResult(
                    call_id=tool_call.call_id,
                    name=tool_call.name,
                    result="Tool execution skipped: task was marked as complete but verification failed.",
                    success=False,
                    error="Task incomplete - tool execution was skipped",
                    id=tool_call.id,
                )
                messages.append(LLMMessage(role="tool", tool_result=tool_result))

        # Add the task incomplete message
        messages.append(LLMMessage(role="user", content=self.task_incomplete_message()))
        return messages
    ...
    async def _tool_call_handler(
        self, tool_calls: list[ToolCall] | None, step: AgentStep, llm_response: "LLMResponse"
    ) -> list[LLMMessage]:
        messages: list[LLMMessage] = []
        if not tool_calls or len(tool_calls) <= 0:
            messages = [
                LLMMessage(
                    role="user",
                    content="It seems that you have not completed the task, please continue and **solve task by calling the appropriate tool(s)**",
                )
            ]
            return messages

        # Deduplicate tool calls to prevent redundant execution
        ...
        if tool_calls:
            ...
            if len(tool_calls) < original_count:
                deduplicated_count = original_count - len(tool_calls)
                print(f"[INFO] Deduplicated {deduplicated_count} redundant tool calls ({original_count} → {len(tool_calls)})")
