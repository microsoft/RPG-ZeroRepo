...
class DockerManager:
    ...
    def execute(self, command, timeout: int = 120) -> tuple[int, str]:
        """
        command: either str (existing pexpect-based behavior) OR list (direct docker exec_run behavior)
        """
        if not self.container:
            raise RuntimeError("Container is not running. Call start() first.")
        if isinstance(command, list):
            # Use direct exec_run (no shell)
            try:
                res = self.container.exec_run(cmd=command, tty=False, stdin=False)
                exit_code = res.exit_code if hasattr(res, "exit_code") else res[0]
                output_bytes = res.output if hasattr(res, "output") else res[1]
                output = output_bytes.decode("utf-8", errors="replace").strip() if isinstance(output_bytes, bytes) else str(output_bytes)
                return exit_code, output
            except Exception as e:
                return -1, f"exec_run error: {e}"
        else:
            # 保持原来的 pexpect string path
            return self._execute_interactive(str(command), timeout)
    ...
    def _execute_interactive(self, command: str, timeout: int) -> tuple[int, str]:
        ...
        # Use a more robust approach similar to bash_tool.py
        # Wrap command in subshell and ensure marker is printed with correct exit code
        marker_command = f"echo {marker}$?"
        
        # For complex commands with pipes/redirects, use a more robust approach:
        # 1. Execute command in subshell to capture exit code properly
        # 2. Use newlines to avoid issues with very long commands
        # 3. Ensure marker is always printed regardless of command output
        print(f"[DEBUG] Executing complex command: {full_command[:100]}..." if len(full_command) > 100 else f"[DEBUG] Executing command: {full_command}")
        self.shell.sendline("(")
        self.shell.sendline(full_command)
        self.shell.sendline(f"); {marker_command}")
        try:
            self.shell.expect(marker + r"(\d+)", timeout=timeout)
        except pexpect.exceptions.TIMEOUT:
            print(f"[DEBUG] Docker command timed out: '{command}' after {timeout}s")
            print(f"[DEBUG] Shell state before timeout: {self.shell.before}")
            return (
                -1,
