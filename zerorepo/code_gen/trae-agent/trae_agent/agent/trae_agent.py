...
class TraeAgent(BaseAgent):
    ...
    def new_task(
        ...
        if "issue" in extra_args:
            ...
            user_message +=  f"[Task statement]: We're currently solving the following issue within our repository. Here's the issue text:\n{extra_args['issue']}\n"
    ...
    def get_git_diff(self) -> str:
        """Get the git diff of the project (including untracked files).

        策略：
        1. 临时 `git add -A`，把工作区所有改动（含新文件）放进 index。
        2. 用 `git diff --cached <base_or_HEAD>` 生成 patch。
        3. 再执行 `git reset` 把 index 恢复为调用前的状态（best-effort）。
        """
        pwd = os.getcwd()
        ...
        try:
            # 添加 safe.directory 以避免 Docker 容器内的 dubious ownership 错误
            subprocess.run(
                ["git", "config", "--global", "--add", "safe.directory", self.project_path],
                check=False,
                capture_output=True,
            )

            # 清理可能残留的 index.lock 文件，防止之前中断的 git 操作阻塞
            index_lock = os.path.join(self.project_path, ".git", "index.lock")
            ...
            # 先检测当前 repo 是否有 HEAD（有没有 commit）
            has_head = True
            try:
            ...
            # 1) 临时把所有改动（含 untracked）加到 index
            subprocess.check_call(["git", "add", "-A"])

            # 2) 组装 diff 命令：优先对比 base_commit；否则对比 HEAD；再不行就对比空树
            if self.base_commit:
                cmd = ["git", "--no-pager", "diff", "--cached", self.base_commit]
            else:
                if has_head:
                    cmd = ["git", "--no-pager", "diff", "--cached", "HEAD"]
                else:
                    # 没有任何 commit 的情况下，就 diff index vs 空树
            ...
            # 3) 尝试把 staging 区恢复回去（若没有 HEAD 可能会失败，就忽略错误）
            try:
                subprocess.run(["git", "reset"], check=False)
            except Exception:
                pass

            # 4) 修复整个项目目录权限，避免容器内 root 创建的文件导致 host 无法写入
            if os.path.exists(self.project_path):
                try:
                    subprocess.run(["chmod", "-R", "a+rw", self.project_path], check=False, capture_output=True)
                except Exception:
                    pass
