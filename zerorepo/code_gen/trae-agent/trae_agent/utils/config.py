...
class ModelConfig:
    ...
    def should_use_max_completion_tokens(self) -> bool:
        ...
        # print("Checking if max_completion_tokens should be used...")
        # print("Max Completion Tokens:", self.max_completion_tokens)
        # print("Model Provider:", self.model_provider.provider)
        ...
        if models is not None and len(models.keys()) > 0:
            ...
            for model_name, model_config in models.items():
                if model_config["model_provider"] not in config_model_providers:
                    raise ConfigError(f"Model provider {model_config['model_provider']} not found: {list(config_model_providers.keys())}")
