...
def build_call_graph(
    ...
    def resolve_unit_key(name: str, file_path: str = "") -> Optional[str]:
        exact_key = f"{file_path}::{name}" if file_path else ""
        if exact_key in unit_to_file:
            return exact_key

        aliases = [name]
        qualified_tail = name.rsplit(".", 1)[-1]
        if qualified_tail != name:
            aliases.append(qualified_tail)

        candidates = []
        for alias in aliases:
            for key in name_to_keys.get(alias, []):
                if key not in candidates:
                    candidates.append(key)
        if file_path:
            for key in candidates:
                if unit_to_file.get(key) == file_path:
                    return key
    ...
    for edge in enhanced_data_flow.get("invocation_edges", []):
        ...
        caller_key = resolve_unit_key(caller, caller_file)
        callee_key = resolve_unit_key(callee, callee_file)
