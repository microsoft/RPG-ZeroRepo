...
class CopilotSessionManager(SessionManager):
    ...
    Model-only calls expose no tools or workspace instructions. Explicit
    agentic calls retain full tool access for code generation and editing.
    """

    def before(self, ctx: TraceContext, prompt: str) -> None:
        """Inject Copilot-specific CLI flags and append prompt.

        Adds tracing, permission, and prompt arguments to ``extra_args``.
        """
        ...
        if self.agentic:
            ctx.extra_args.append("--allow-all")
        else:
            ctx.extra_args.extend([
                "--silent",
                "--available-tools=",
        ...
        ctx.extra_args.extend(["-p", prompt])
