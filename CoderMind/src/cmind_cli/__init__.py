...
from . import _codex_config, _codex_skills, _storage, _workspace_config
...
#   * CMIND_AI[ai]    → always injected for the selected LLM backend.
#
...
""",
    "codex": """\
# Codex skill definitions (regenerated by cmind)
.agents/skills/cmind-*/
""",
}

_INTEGRATED_AGENTS = ("claude", "copilot", "codex")
...
def _setup_gitignore(project_path: Path, selected_ai: str) -> None:
    ...
    * **Existing repo or existing ``.gitignore``** — do not overwrite
    ...
    Args:
        ...
        selected_ai: Active LLM backend. Generated integration ignores are
            backend-independent; retained for API compatibility.
    """
    gitignore = project_path / ".gitignore"
    git_dir = project_path / ".git"

    cmind_block = _GITIGNORE_CMIND_COMMON
    for agent in _INTEGRATED_AGENTS:
        ai_rules = _GITIGNORE_CMIND_AI.get(agent)
        if ai_rules:
            cmind_block += "\n" + ai_rules
...
def _generate_mcp_config(
    ...
    """Generate project-scoped MCP configuration for all integrated agents.
    ...
    against the bundled scripts that match the installed CLI version.
        ...
        - Codex:   ``.codex/config.toml`` (key ``mcp_servers.rpg-tools``)
    ...
    try:
        ...
        claude_data["mcpServers"]["rpg-tools"] = mcp_server_config
    ...
    try:
        ...
        copilot_data = _load_json_dict(copilot_file)
        copilot_data.setdefault("servers", {})
        copilot_data["servers"]["rpg-tools"] = mcp_server_config
        with open(copilot_file, "w", encoding="utf-8") as file:
...
def _install_hooks(
    ...
    """Install RPG auto-update hooks for all integrated assistants.
...
def _install_from_bundle(
    ...
    *,
    ...
    try:
        ...
        # 1. Materialise every supported agent integration. ``ai_assistant``
        #    selects the encoder/decoder backend, not which integrations exist.
        for agent in _INTEGRATED_AGENTS:
            _materialise_commands_for_agent(
                agent, _assets.commands_dir(), project_path
            )
...
def _materialise_commands_for_agent(
    ...
    Layout produced:
      claude  → ``.claude/commands/cmind.<name>.md``
            ...
            codex   → ``.agents/skills/cmind-<name>/SKILL.md``
      others  → fallback: ``.cmind/commands/cmind.<name>.md`` (same
                ``cmind.<name>.md`` prefix for consistency with the
                supported agents above)

    NOTE: ``claude``, ``copilot``, and ``codex`` have explicit layouts.
    Add new agents here when AGENT_CONFIG grows.
    """
    ...
    if ai_assistant == "claude":
        ...
        _codex_skills.materialize_skills(src_commands_dir, project_path)
    else:
...
def init(
    ...
    ai_assistant: str = typer.Option(
        None,
        "--ai",
        help="Default encoder/decoder backend: copilot, claude, or codex",
    ),
    ...
    2. Let you choose the default encoder/decoder backend
    3. Install Claude, Copilot, and Codex integrations from the bundle
    4. Place all integrations into the project
    ...
    if ai_assistant:
        ...
        if sys.stdin.isatty():
            ...
            selected_ai = select_with_arrows(
                ai_choices, "Choose the encoder/decoder backend:", "copilot"
            )
        else:
            selected_ai = "copilot"
    ...
    console.print(f"[cyan]Selected LLM backend:[/cyan] {selected_ai}")
    ...
    tracker.add("ai-select", "Select LLM backend")
    ...
    with Live(
        ...
        try:
            ...
            # Global registration for Copilot CLI, independent of which
            # encoder/decoder backend is active.
            if no_mcp:
                pass
    ...
    security_notice = Panel(
        "CoderMind regenerates Claude commands, Copilot prompts, and Codex "
        "skills under [cyan].claude/commands/[/cyan], "
        "[cyan].github/{agents,prompts}/[/cyan], and "
        "[cyan].agents/skills/cmind-*/[/cyan]. These generated files are "
        "excluded from git by default.\nCollaborators should run "
        "[cyan]cmind init .[/cyan] in their clone to materialize them locally.",
        title="[yellow]Agent Integration Notice[/yellow]",
        border_style="yellow",
        padding=(1, 2),
    )
    console.print()
    ...
    steps_lines.append(
        f"{step_num}. Use [cyan]/cmind.*[/cyan] in Claude/Copilot or "
        "[cyan]$cmind-*[/cyan] in Codex:"
    )
...
def update(
    ai_assistant: str = typer.Option(
        None,
        "--ai",
        help="Change the default encoder/decoder backend: copilot, claude, or codex",
    ),
    script_type: str = typer.Option(
        None, "--script", help="Script type to use: sh or ps"
    ),
    ...
    It preserves the active encoder/decoder backend unless ``--ai`` is passed.
    ...
    # Determine the active encoder/decoder backend.
    if ai_assistant:
        if ai_assistant not in AGENT_CONFIG:
            console.print(
        ...
        try:
            backend = _workspace_config.read_active_backend(project_path)
        except _workspace_config.WorkspaceConfigError as exc:
            console.print(f"[red]Error:[/red] {exc}")
            raise typer.Exit(1) from exc
        if backend.agent is None:
            console.print(
                f"[red]Error:[/red] Active backend command "
                f"{backend.command!r} is custom. Re-run with "
                "[cyan]--ai copilot|claude|codex[/cyan] to select a supported backend."
            )
            raise typer.Exit(1)
        selected_ai = backend.agent
        console.print(f"[cyan]Active LLM backend:[/cyan] {selected_ai}")
    ...
    with Live(
        ...
        try:
            ...
            # Global registration for Copilot CLI, independent of the active
            ...
            if ai_assistant:
                _workspace_config.set_active_backend(project_path, selected_ai)
    ...
    console.print(
        f"[dim]Updated: scripts and Claude/Copilot/Codex integrations in "
